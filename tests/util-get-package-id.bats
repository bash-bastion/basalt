...
@test "Fails (remote) with no site" {
	...
	assert_failure
	assert_line -p "Fatal: Argument 'site' for function 'util.get_package_id' is empty"
}
...
@test "Fails (remote) with no version" {
	...
	assert_failure
	assert_line -p "Fatal: Argument 'version' for function 'util.get_package_id' is empty"
}
