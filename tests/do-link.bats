...
@test "fails with an invalid path" {
	run basalt global link invalid
...
@test "fails if not a Git repository" {
	...
	run basalt global link "$BASALT_ORIGIN_DIR/$pkg"
...
@test "fails if package already present (as erroneous file)" {
	...
	mkdir -p touch "$BASALT_PACKAGES_PATH/local"
	touch "$BASALT_PACKAGES_PATH/local/theta"

	test_util.create_package 'theta'
	mkdir 'theta'

	run basalt global link "$BASALT_ORIGIN_DIR/theta"
...
@test "links the package to packages under the correct namespace (local)" {
	...
	run basalt global link "$BASALT_ORIGIN_DIR/$dir"

	assert_success
	assert [ "$(readlink -f $BASALT_PACKAGES_PATH/local/package1)" = "$(readlink -f "$BASALT_ORIGIN_DIR/$dir")" ]
}
...
@test "calls link-bins, link-completions, link-man and deps in order" {
	...
	local srcDir="$(util.readlink "$BASALT_ORIGIN_DIR/$dir")"

	run basalt global link "$srcDir"
...
@test "calls link-bins, link-completions, link-man and deps in order for multiple directories" {
	...
	local srcDir1="$(util.readlink "$BASALT_ORIGIN_DIR/$dir1")"
	local srcDir2="$(util.readlink "$BASALT_ORIGIN_DIR/$dir2")"

	run basalt global link "$srcDir1" "$srcDir2"
...
@test "links the current directory" {
	...
	cd "$BASALT_ORIGIN_DIR/$dir"
	run basalt global link .

	assert_success
	assert [ "$(readlink -f "$BASALT_PACKAGES_PATH/local/$dir")" = "$(readlink -f "$BASALT_ORIGIN_DIR/$dir")" ]
}
...
@test "links an arbitrary complex relative path" {
	...
	cd "$BASALT_ORIGIN_DIR/parent"
	run basalt global link "./$dir/.././$dir"

	assert_success
	assert [ "$(readlink -f "$BASALT_PACKAGES_PATH/local/$dir")" = "$(readlink -f "$BASALT_ORIGIN_DIR/parent/$dir")" ]
}
